...
from rlsp.agents.rlsp_td3 import RLSPTD3Agent
import gym
import numpy as np
...
def execute(agent_helper):
    ...
    agent = create_agent(agent_helper.env, agent_helper.config, agent_helper.result, logger)
    agent_helper.agent = agent
...
def train_agent(agent_helper):
    ...
    training(agent_helper.agent, agent_helper.env, agent_helper.callbacks, agent_helper.episodes, agent_helper.result, agent_helper.episode_steps)
    agent_helper.result.runtime_stop()
    # Save weights to the nested directory
    weights_path = f"{agent_helper.result_base_path}/{agent_helper.experiment_id}/weights"
    os.makedirs(weights_path, exist_ok=True)
    logger.info(f"Saving weights to {weights_path}")
    agent_helper.agent.save_weights(weights_path, overwrite=True)
...
def training(agent, env, callbacks, episodes, result, episode_steps):
    ...
    for episode in range(episodes):
        ...
        for step in range(episode_steps):
            ...
            agent.train(observation, action, reward, next_observation, done)
        ...
        # Log episode results
        logger.info(f"Episode {episode + 1}/{episodes} - Reward: {episode_reward}")
...
def create_agent(env, config, result, logger):
    """Create the appropriate agent based on agent_type"""
    agent_type = config['agent_type']
    if agent_type == "DDPG":
        from rlsp.agents.rlsp_ddpg import RLSPDDPGAgent
        agent = RLSPDDPGAgent()
    elif agent_type == "TD3":
        from rlsp.agents.rlsp_td3 import RLSPTD3Agent
        agent = RLSPTD3Agent()
    ...
    agent.create(env, config, result, logger)
    return agent


if __name__ == '__main__':
    agent_config = 'res/config/agent/td3/agent_obs1_prio-flow_64a_64c_099gam_00001tau_001alp_0001dec.yaml'
